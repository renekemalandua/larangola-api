...
> {
  ...
  async execute(
    request: CreateAgentSubscriptionRequestDTO
...
@Injectable()
export class UpdateAgentSubscriptionUseCase implements UseCase<
  { id: string; data: UpdateAgentSubscriptionRequestDTO },
  AgentSubscriptionEntity
> {
  ...
  async execute({
    ...
    id: string;
    data: UpdateAgentSubscriptionRequestDTO;
  }): Promise<AgentSubscriptionEntity> {
    const entity = await this.repository.findById(id);
    if (!entity) throw new BadRequestException('Agent subscription not found');
    if (data.endDate !== undefined)
      entity.endDate = data.endDate ? new Date(data.endDate) : null;
...
@Injectable()
export class ListAgentSubscriptionsUseCase implements UseCase<
  void,
  AgentSubscriptionEntity[]
> {
  constructor(private readonly repository: IAgentSubscriptionRepository) {}
...
@Injectable()
export class ListAgentSubscriptionsByAgentUseCase implements UseCase<
  string,
  AgentSubscriptionEntity[]
> {
  constructor(private readonly repository: IAgentSubscriptionRepository) {}
...
@Injectable()
export class FindAgentSubscriptionByIdUseCase implements UseCase<
  string,
  AgentSubscriptionEntity | null
> {
  constructor(private readonly repository: IAgentSubscriptionRepository) {}
