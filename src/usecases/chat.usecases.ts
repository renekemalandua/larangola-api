...
> {
  ...
  async execute(request: CreateChatRequestDTO): Promise<ChatEntity> {
    ...
    const existing = await this.repository.findByUsers(
      request.user1Id,
      request.user2Id
    );
    if (existing)
...
@Injectable()
export class UpdateChatUseCase implements UseCase<
  { id: string; data: UpdateChatRequestDTO },
  ChatEntity
> {
  ...
  async execute({
    ...
    if (data.lastMessage !== undefined)
      entity.lastMessage = data.lastMessage ?? null;
    if (data.isBlocked !== undefined) entity.isBlocked = data.isBlocked;
    if (data.unreadCountUser1 !== undefined)
      entity.unreadCountUser1 = data.unreadCountUser1;
    if (data.unreadCountUser2 !== undefined)
      entity.unreadCountUser2 = data.unreadCountUser2;
...
@Injectable()
export class FindChatByUsersUseCase implements UseCase<
