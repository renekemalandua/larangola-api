...
import {
  ...
  UpdateClosedDealRequestDTO,
} from '../dto/closed-deal.dto';

@Injectable()
export class CreateClosedDealUseCase implements UseCase<
  CreateClosedDealRequestDTO,
  ClosedDealEntity
> {
  constructor(
...
@Injectable()
export class UpdateClosedDealUseCase implements UseCase<
...
> {
  ...
  async execute({
    ...
    if (data.closedDate !== undefined)
      entity.closedDate = new Date(data.closedDate);
...
@Injectable()
export class ListClosedDealsUseCase implements UseCase<
  void,
  ClosedDealEntity[]
> {
  constructor(private readonly repository: IClosedDealRepository) {}
...
@Injectable()
export class ListClosedDealsByAgentUseCase implements UseCase<
  string,
  ClosedDealEntity[]
> {
  constructor(private readonly repository: IClosedDealRepository) {}
...
@Injectable()
export class ListClosedDealsByClientUseCase implements UseCase<
  string,
  ClosedDealEntity[]
> {
  constructor(private readonly repository: IClosedDealRepository) {}
...
@Injectable()
export class FindClosedDealByIdUseCase implements UseCase<
  string,
