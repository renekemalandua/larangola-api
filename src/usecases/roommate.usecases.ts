...
@Injectable()
export class CreateRoommateUseCase implements UseCase<
  CreateRoommateRequestDTO,
  RoommateEntity
> {
  constructor(
  ...
  async execute(request: CreateRoommateRequestDTO): Promise<RoommateEntity> {
    ...
    if (existing)
      throw new BadRequestException('Roommate already exists for this user');
...
@Injectable()
export class UpdateRoommateUseCase implements UseCase<
  { id: string; data: UpdateRoommateRequestDTO },
  RoommateEntity
> {
  ...
  async execute({
    ...
    id: string;
    data: UpdateRoommateRequestDTO;
  }): Promise<RoommateEntity> {
    ...
    if (data.profession !== undefined)
      entity.profession = data.profession ?? null;
    ...
    if (data.moveInDate !== undefined)
      entity.moveInDate = data.moveInDate ? new Date(data.moveInDate) : null;
    ...
    if (data.responseRate !== undefined)
      entity.responseRate = data.responseRate ?? null;
    if (data.averageResponseTime !== undefined)
      entity.averageResponseTime = data.averageResponseTime ?? null;
    if (data.rating !== undefined) entity.rating = data.rating ?? null;
    if (data.lookingForRoommate !== undefined)
      entity.lookingForRoommate = data.lookingForRoommate;
    if (data.preferences !== undefined)
      entity.preferences = data.preferences ?? null;
...
@Injectable()
export class FindRoommateByIdUseCase implements UseCase<
  string,
  RoommateEntity | null
> {
  constructor(private readonly repository: IRoommateRepository) {}
...
@Injectable()
export class FindRoommateByUserIdUseCase implements UseCase<
  string,
  RoommateEntity | null
> {
  constructor(private readonly repository: IRoommateRepository) {}
