...
@Injectable()
export class UpdateScheduledVisitUseCase implements UseCase<
...
> {
  ...
  async execute({
    ...
    id: string;
    data: UpdateScheduledVisitRequestDTO;
  }): Promise<ScheduledVisitEntity> {
    const entity = await this.repository.findById(id);
    if (!entity) throw new BadRequestException('Scheduled visit not found');
    if (data.scheduledDate !== undefined)
      entity.scheduledDate = new Date(data.scheduledDate);
...
@Injectable()
export class ListScheduledVisitsUseCase implements UseCase<
  void,
  ScheduledVisitEntity[]
> {
  constructor(private readonly repository: IScheduledVisitRepository) {}
...
@Injectable()
export class ListScheduledVisitsByUserUseCase implements UseCase<
  string,
  ScheduledVisitEntity[]
> {
  constructor(private readonly repository: IScheduledVisitRepository) {}
...
@Injectable()
export class FindScheduledVisitByIdUseCase implements UseCase<
  string,
