...
@Injectable()
export class CreateUserUseCase implements UseCase<CreateUserRequestDTO, UserEntity> {
...
@Injectable()
export class UpdateUserUseCase implements UseCase<{ id: string; data: UpdateUserRequestDTO }, UserEntity> {
	constructor(private readonly repository: IUserRepository) { }
	async execute({ id, data }: { id: string; data: UpdateUserRequestDTO }): Promise<UserEntity> {
		...
		if (data.phone !== undefined) {
			...
			if (existing && existing.id !== id) throw new BadRequestException('Phone already exists');
...
@Injectable()
export class FindUserByEmailUseCase implements UseCase<string, UserEntity | null> {
	constructor(private readonly repository: IUserRepository) { }
