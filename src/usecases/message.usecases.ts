...
@Injectable()
export class CreateMessageUseCase implements UseCase<
  CreateMessageRequestDTO,
  MessageEntity
> {
  constructor(
  ...
  async execute(request: CreateMessageRequestDTO): Promise<MessageEntity> {
    ...
    if (
      chat.user1Id !== request.senderId &&
      chat.user2Id !== request.senderId
    ) {
...
@Injectable()
export class UpdateMessageUseCase implements UseCase<
  { id: string; data: UpdateMessageRequestDTO },
  MessageEntity
> {
...
@Injectable()
export class ListMessagesByChatUseCase implements UseCase<
  string,
  MessageEntity[]
> {
  constructor(private readonly repository: IMessageRepository) {}
...
@Injectable()
export class FindMessageByIdUseCase implements UseCase<
  string,
  MessageEntity | null
> {
  constructor(private readonly repository: IMessageRepository) {}
...
@Injectable()
export class MarkMessagesAsReadUseCase implements UseCase<
