...
> {
    ...
    async execute(request: CreatePropertyRequestDTO): Promise<PropertyRequestEntity> {
...
@Injectable()
export class ListPropertyRequestsUseCase implements UseCase<void, PropertyRequestEntity[]> {
    constructor(private readonly repository: IPropertyRequestRepository) { }
...
> {
    ...
    async execute({
        ...
        if (data.propertyType !== undefined) entity.propertyType = data.propertyType ?? '';
