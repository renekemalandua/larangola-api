...
export class PrismaMessageRepository implements IMessageRepository {
	...
	async list(): Promise<MessageEntity[]> {
		const rows = await this.prisma.message.findMany({ orderBy: { createdAt: 'desc' } });
	...
	async update(data: MessageEntity): Promise<MessageEntity> {
		const exists = await this.prisma.message.findUnique({ where: { id: data.id } });
		if (!exists) throw new NotFoundException('Message not found');
		const raw = MessageAdapter.toPrisma(data) as any;
		const updated = await this.prisma.message.update({ where: { id: data.id }, data: raw });
