...
export class PrismaAgentRepository implements IAgentRepository {
	...
	async list(): Promise<AgentEntity[]> {
		const rows = await this.prisma.agent.findMany({ orderBy: { updatedAt: 'desc' } });
	...
	async update(data: AgentEntity): Promise<AgentEntity> {
		const exists = await this.prisma.agent.findUnique({ where: { id: data.id } });
		if (!exists) throw new NotFoundException('Agent not found');
		const raw = AgentAdapter.toPrisma(data) as any;
		const updated = await this.prisma.agent.update({ where: { id: data.id }, data: raw });
