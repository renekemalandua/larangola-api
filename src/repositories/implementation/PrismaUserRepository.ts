...
export class PrismaUserRepository implements IUserRepository {
	...
	async list(): Promise<UserEntity[]> {
		const rows = await this.prisma.user.findMany({ orderBy: { updatedAt: 'desc' } });
	...
	async update(data: UserEntity): Promise<UserEntity> {
		const exists = await this.prisma.user.findUnique({ where: { id: data.id } });
		if (!exists) throw new NotFoundException('User not found');
		const raw = UserAdapter.toPrisma(data) as any;
		const updated = await this.prisma.user.update({ where: { id: data.id }, data: raw });
