...
import { CreatePropertyCategoryRequestDTO, UpdatePropertyCategoryRequestDTO } from '../dto/property-category.dto';
...
export class PropertyCategoryController {
	...
	async list(@Res() response) {
		try {
			...
			const data = entities.map((e) => PropertyCategoryAdapter.toHttp(e));
	...
	async delete(@Param('id') id: string, @Res() response) {
		try {
			...
			return response.status(200).json({ status: true, data: { message: 'Property category deleted successfully' } });
		} catch (error) {
			throw new BadRequestException(error.message);
		}
	}
