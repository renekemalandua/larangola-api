...
export class UserController {
	...
	@ApiResponse({ status: 200 })
	@ApiResponse({ status: 400, type: HttpErrorResponseDTO })
	async update(@Param('id') id: string, @Body() body: UpdateUserRequestDTO, @Res() response) {
		try {
	...
	async delete(@Param('id') id: string, @Res() response) {
		try {
			...
			return response.status(200).json({ status: true, data: { message: 'User deleted successfully' } });
		} catch (error) {
			throw new BadRequestException(error.message);
		}
	}
