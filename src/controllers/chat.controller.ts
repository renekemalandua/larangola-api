import { BadRequestException, Body, Controller, Delete, Get, Param, Post, Put, Res, Query } from '@nestjs/common';
...
export class ChatController {
	...
	@ApiResponse({ status: 200 })
	@ApiResponse({ status: 400, type: HttpErrorResponseDTO })
	async findByUsers(@Param('user1Id') user1Id: string, @Param('user2Id') user2Id: string, @Res() response) {
		try {
			const entity = await this.findByUsersUseCase.execute({ user1Id, user2Id });
			const data = ChatAdapter.toHttp(entity!);
			return response.status(200).json({ status: true, data });
		} catch (error) {
			throw new BadRequestException(error.message);
	...
	@ApiResponse({ status: 200 })
	@ApiResponse({ status: 400, type: HttpErrorResponseDTO })
	async update(@Param('id') id: string, @Body() body: UpdateChatRequestDTO, @Res() response) {
		try {
	...
	async delete(@Param('id') id: string, @Res() response) {
		try {
			...
			return response.status(200).json({ status: true, data: { message: 'Chat deleted successfully' } });
		} catch (error) {
			throw new BadRequestException(error.message);
		}
	}
