...
export class ReviewController {
	...
	@ApiResponse({ status: 200 })
	@ApiResponse({ status: 400, type: HttpErrorResponseDTO })
	async update(@Param('id') id: string, @Body() body: UpdateReviewRequestDTO, @Res() response) {
		try {
	...
	async delete(@Param('id') id: string, @Res() response) {
		try {
			...
			return response.status(200).json({ status: true, data: { message: 'Review deleted successfully' } });
		} catch (error) {
			throw new BadRequestException(error.message);
		}
	}
