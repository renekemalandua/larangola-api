...
import { CreateUserUseCase, UpdateUserUseCase, DeleteUserUseCase, ListUsersUseCase, FindUserByIdUseCase, FindUserByEmailUseCase } from '../../../src/usecases/user.usecases';
...
describe('User Use Cases (Unit)', () => {
	...
	describe('CreateUserUseCase', () => {
		it('should create a user successfully', async () => {
			...
			const mockEntity = UserEntity.create(request, new IdValueObject('user-id-1'));
		...
		it('should throw error if email already exists', async () => {
			...
			const existingUser = UserEntity.create(request, new IdValueObject('existing-id'));

			mockRepository.findByEmail.mockResolvedValue(existingUser);

			await expect(createUseCase.execute(request)).rejects.toThrow(BadRequestException);
	...
	describe('UpdateUserUseCase', () => {
		it('should update user successfully', async () => {
			...
			const result = await updateUseCase.execute({ id: 'user-id-1', data: updateData });
		...
		it('should throw error if user not found', async () => {
			...
			await expect(
				updateUseCase.execute({ id: 'non-existent', data: { name: 'New Name' } })
	...
	describe('DeleteUserUseCase', () => {
		...
		it('should throw error if user not found', async () => {
			...
			await expect(deleteUseCase.execute('non-existent')).rejects.toThrow(BadRequestException);
		});
	});
	...
	describe('FindUserByIdUseCase', () => {
		...
		it('should throw error if user not found', async () => {
			...
			await expect(findByIdUseCase.execute('non-existent')).rejects.toThrow(BadRequestException);
		});
	});
	...
	describe('FindUserByEmailUseCase', () => {
		...
		it('should throw error if user not found', async () => {
			...
			await expect(findByEmailUseCase.execute('notfound@example.com')).rejects.toThrow(
				BadRequestException
			);
		});
	});
